...
contract('KlickZieIco', function(accounts) {

  it("should not be halted when created", function(done) {
      KlickZieIco.new(accounts[2], 1234, accounts[1], 8000 * Math.pow(10, 18), 8000 * Math.pow(10, 18), 120000 * Math.pow(10, 18), {from: accounts[0]}).then(function(instance) {
  ...
  it("should be halted and then unhalted", function(done) {
      var soma;
      KlickZieIco.new(accounts[0], 1234, accounts[1], 8000 * Math.pow(10, 18), 8000 * Math.pow(10, 18), 120000 * Math.pow(10, 18), {from: accounts[0]}).then(function(instance) {
      soma = instance;
  ...
  it("should convert 1 ETH to 450 * 125% in PreSale", function(done) {
    ...
    KlickZieIco.new(accounts[0], currentBlockTimeStamp - 90, accounts[5], 8000 * Math.pow(10, 18), 8000 * Math.pow(10, 18), 120000 * Math.pow(10, 18), {from: accounts[0]}).then(function(instance) {
  ...
  it("should fail as preSale did not start yet", function(done) {
    var currentBlockTimeStamp = web3.eth.getBlock(web3.eth.blockNumber).timestamp;
    var soma;
    KlickZieIco.new(accounts[0], currentBlockTimeStamp + 60*60, accounts[2], 8000 * Math.pow(10, 18), 8000 * Math.pow(10, 18), 120000 * Math.pow(10, 18), {from: accounts[0]}).then(function(instance) {
      soma = instance;
      return soma.isPreSalePeriod();
    }).then(function(isPreSalePeriod) {
  ...
  it("should fail as preSale is over but ICO dates were not set yet", function(done) {
    var currentBlockTimeStamp = web3.eth.getBlock(web3.eth.blockNumber).timestamp;
    var soma;
    KlickZieIco.new(accounts[0], currentBlockTimeStamp - 90, accounts[2], 8000 * Math.pow(10, 18), 8000 * Math.pow(10, 18), 120000 * Math.pow(10, 18), {from: accounts[0]}).then(function(instance) {
      soma = instance;
  ...
  it("should fail when target for PreSale is reached in PreSale phase", function(done) {
    var currentBlockTimeStamp = web3.eth.getBlock(web3.eth.blockNumber).timestamp;
    var soma;
    MarksIco.new(accounts[0], currentBlockTimeStamp - 90, accounts[6], 2 * Math.pow(10, 18), 2 * Math.pow(10, 18), 2 * Math.pow(10, 18), {from: accounts[0]}).then(function(instance) {
      soma = instance;
  ...
  it("should fail when max target for ICO is reached in ICO phase", function(done) {
    var currentBlockTimeStamp = web3.eth.getBlock(web3.eth.blockNumber).timestamp;
    var soma;
    KlickZieIco.new(accounts[0], currentBlockTimeStamp - 90, accounts[6], 2 * Math.pow(10, 18), 2 * Math.pow(10, 18), 3 * Math.pow(10, 18), {from: accounts[0]}).then(function(instance) {
      soma = instance;
      // PreSale Phase:
      ...
      assert.equal(result.logs[0].args.tokens, parseInt(Math.pow(10, 18) * 450 * 1.25 * 1.5), "Incorrect number of MARKS tokens");
      ...
      assert.equal(result.logs[0].args.tokens, 4 * 450 * Math.pow(10, 18), "Incorrect number of MARKS tokens");
  ...
  it("should calculate proper bonuses in various days of the PreSale phase", function(done) {
    var currentBlockTimeStamp = web3.eth.getBlock(web3.eth.blockNumber).timestamp;
    var soma;
    KlickZieIco.new(accounts[0], currentBlockTimeStamp - 90, accounts[6], 200 * Math.pow(10, 18), 200 * Math.pow(10, 18), 300 * Math.pow(10, 18), {from: accounts[0]}).then(function(instance) {
      soma = instance;
      // PreSale Phase 1:
      return soma.sendTransaction({ from: accounts[1], value: web3.toWei(1, "ether") });
    }).then(function(result) {
      ...
      assert.equal(result.logs[0].args.tokens, parseInt(Math.pow(10, 18) * 450 * 1.25 * 1), "Incorrect number of MARKS tokens");
      ...
      assert.equal(result.logs[0].args.weiAmount, 1 * Math.pow(10, 18), "Incorrect number of weiAmount");
      assert.equal(result.logs[0].args.tokens, parseInt(Math.pow(10, 18) * 450 * 1.20 * 1), "Incorrect number of MARKS tokens");
      ...
      assert.equal(result.logs[0].args.weiAmount, 1 * Math.pow(10, 18), "Incorrect number of weiAmount");
      assert.equal(result.logs[0].args.tokens, Math.pow(10, 17) * parseInt(10 * 450 * 1.15 * 1), "Incorrect number of MARKS tokens");
      ...
      assert.equal(result.logs[0].args.weiAmount, 1 * Math.pow(10, 18), "Incorrect number of weiAmount");
      assert.equal(result.logs[0].args.tokens, Math.pow(10, 17) * parseInt(10 * 450 * 1.10 * 1), "Incorrect number of MARKS tokens");
      // Fastforward 7 more days:
      return solidityTestUtil.evmIncreaseTime(7 * 24 * 60 * 60);
      }).then(function() {
      // PreSale Phase 5:
      ...
      assert.equal(result.logs[0].args.weiAmount, 1 * Math.pow(10, 18), "Incorrect number of weiAmount");
      assert.equal(result.logs[0].args.tokens, Math.pow(10, 17) * parseInt(10 * 450 * 1.05 * 1), "Incorrect number of MARKS tokens");
      ...
      assert.equal(result.logs[0].args.recipient, accounts[6], "Incorrect recipient");
      assert.equal(result.logs[0].args.weiAmount, 1 * Math.pow(10, 18), "Incorrect number of weiAmount");
      assert.equal(result.logs[0].args.tokens, parseInt(Math.pow(10, 18) * 450 * 1.05 * 1), "Incorrect number of MARKS tokens");
    }).then(function(result) {
      return soma.totalSupply();
    }).then(function(totalSupply) {
      // divided by Math.pow(10, 5) to still be within JS int scope:
  ...
  it("should calculate proper bonuses also if PreSale phase finishes quickly and ICO starts within PreSale period (1)", function(done) {
    var currentBlockTimeStamp = web3.eth.getBlock(web3.eth.blockNumber).timestamp;
    var soma;
    KlickZieIco.new(accounts[0], currentBlockTimeStamp - 90, accounts[6], 2 * Math.pow(10, 18), 3 * Math.pow(10, 18), 5 * Math.pow(10, 18), {from: accounts[0]}).then(function(instance) {
      soma = instance;
      // PreSale Phase 1:
      ...
      assert.equal(result.logs[0].args.tokens.div(Math.pow(10, 5)), parseInt(Math.pow(10, 13) * 450 * 1.25 * 3), "Incorrect number of MARKS tokens");
      // Fastforward 2 moredays:
      return solidityTestUtil.evmIncreaseTime(2 * 24 * 60 * 60);
    }).then(function() {
      ...
      assert.equal(result.logs[0].args.recipient, accounts[2], "Incorrect recipient");
      assert.equal(result.logs[0].args.weiAmount, 1 * Math.pow(10, 18), "Incorrect number of weiAmount");
      assert.equal(result.logs[0].args.tokens.div(Math.pow(10, 5)), parseInt(Math.pow(10, 13) * 450 * 1 * 1), "Incorrect number of MARKS tokens");
    }).then(function(result) {
      return soma.totalSupply();
    }).then(function(totalSupply) {
      // divided by Math.pow(10, 5) to still be within JS int scope:
  ...
  it("should calculate proper bonuses also if PreSale phase finishes quickly and ICO starts within PreSale period (2)", function(done) {
    ...
    KlickZieIco.new(accounts[0], currentBlockTimeStamp - 90, accounts[6], 2 * Math.pow(10, 18), 3 * Math.pow(10, 18), 5 * Math.pow(10, 18), {from: accounts[0]}).then(function(instance) {
      ...
      assert.equal(result.logs[0].args.tokens.div(Math.pow(10, 5)), parseInt(Math.pow(10, 13) * 450 * 1.25 * 2), "Incorrect number of MARKS tokens");
      // Fastforward 2 moredays:
      return solidityTestUtil.evmIncreaseTime(2 * 24 * 60 * 60);
    }).then(function() {
      // set ICO dates to overlap with already maxed preSale period:
